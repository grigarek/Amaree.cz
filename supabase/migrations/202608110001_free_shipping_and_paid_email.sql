...
set discount_type = 'free_shipping', value = 1, currency = null, updated_at = now()
where upper(code) = 'DOPRAVA';

update public.accounting_documents
set last_error_code = null, last_error_message = null, updated_at = now()
where provider = 'fakturoid'
  and last_error_message ilike '%email delivery is only available for paid plans%';
...
begin
  ...
  if nullif(upper(trim(p_payload->>'discountCode')), '') is not null then
    ...
    if found then
      discount_value := case
        when discount_row.discount_type = 'percent' then floor(subtotal_value * discount_row.value / 100.0)::integer
        when discount_row.discount_type = 'fixed' then least(discount_row.value, subtotal_value)
        else 0
      end;
    end if;
  end if;

  if shipping_option.free_from_minor is not null
    and subtotal_value - discount_value >= shipping_option.free_from_minor then
    shipping_value := 0;
    ...
    shipping_value := shipping_option.price_minor;
  end if;
