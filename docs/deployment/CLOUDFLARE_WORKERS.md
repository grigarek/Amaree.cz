...
- staging: `https://amaree-staging.amaree-cz.workers.dev`, development Supabase a pouze testovací/vypnuté integrace,
- production: rezervované nastavení pro `amaree.cz`; bez výslovného schválení se nenasazuje.

Staging `NEXT_PUBLIC_SITE_URL` ve `wrangler.jsonc` odpovídá přidělené `workers.dev` adrese. Tajné hodnoty se nevkládají do souboru ani do GitHubu jako text.
...
- Cloudflare Free vynucuje vlastní hard limity; vlastní `limits` konfigurace není na Free tarifu podporovaná. Integrace jsou ve stagingu vypnuté.
