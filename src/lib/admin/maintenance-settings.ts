...
import { defaultMaintenanceSettings, isMaintenanceActive, parseMaintenanceSettings, type MaintenanceSettings } from "@/lib/maintenance";
...
export async function getMaintenanceSettings(): Promise<MaintenanceSettings> {
  ...
  const settings = parseMaintenanceSettings(data.value);
  return { ...settings, enabled: isMaintenanceActive(settings) };
}
