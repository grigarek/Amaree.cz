      ...
      <main className="mx-auto max-w-page px-5 py-10">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.35fr_0.65fr]">
          <div className="space-y-8">
            ...
            <section><h2 className="font-newsreader text-3xl">Historie e-mailů</h2><div className="mt-4 divide-y divide-line border-y border-line">{order.emails.map((email) => <details className="py-4 font-redhat text-sm" key={email.id}><summary className="grid cursor-pointer gap-2 md:grid-cols-[1fr_auto]"><div><p className="font-semibold">{email.subject}</p><p className="text-xs text-muted">{email.recipient} · {email.templateKey}{email.errorMessage ? ` · ${email.errorMessage}` : ""}</p></div><span className="text-xs font-semibold text-ruby">{email.status}</span></summary><div className="mt-4 grid gap-4"><div><h3 className="text-xs font-semibold uppercase text-ruby">Textová verze</h3><pre className="mt-2 whitespace-pre-wrap text-sm leading-6 text-muted">{email.text}</pre></div><div><h3 className="text-xs font-semibold uppercase text-ruby">Finální HTML</h3><iframe className="mt-2 min-h-96 w-full border border-line bg-white" sandbox="" srcDoc={email.html} title={`Uložený e-mail ${email.subject}`} /></div></div></details>)}{!order.emails.length ? <p className="py-4 font-redhat text-sm text-muted">Zatím nebyl zaznamenán žádný e-mail.</p> : null}</div></section>
